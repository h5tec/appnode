...
RUN         mkdir -p /usr/src/app && \
            ...
            apt-get install -y wget curl nginx python-certbot-nginx net-tools ufw nano mtr htop gnupg2 ca-certificates && \
            apt-get install -y sysvinit-utils dnsutils apt-transport-https software-properties-common && \
            pip install --upgrade pip && \
            pip install --no-cache-dir --upgrade httpx tornado motor pymongo dnspython websocket-client speedtest-cli tormysql qrcode requests ipaddress six && \
